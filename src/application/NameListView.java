...
public class NameListView extends BorderPane {
    ...
    private void remove() {
        boolean confirm = Dialogs.confirmDialog("Remove?", "Remove?", "Are you sure you want to remove this entry?");
        if (confirm) {
            String name = listView.getSelectionModel().getSelectedItem();
            boolean success = phoneBook.remove(name);
            if (success) {
                Dialogs.alert("Success",null,"Entry removed.");
                obsList.remove(name);
                clearSelection();
            } else {
                Dialogs.alert("Failed to remove entry", null, "Entry does not exist.");
            }
        }
    }
    ...
    private void removeNumber() {
        Optional<String> number = Dialogs.oneInputDialog("Enter phone number to remove",null,"Phone number");
        if (number.isPresent()) {
            String name = listView.getSelectionModel().getSelectedItem();
            boolean success = phoneBook.removeNumber(name, number.get());
            if (success) {
                Dialogs.alert("Success",null,"Number removed.");
                numbersLabel.setText(name + " " + phoneBook.findNumbers(name));
            } else {
                Dialogs.alert("Failed to remove number", null, "Number does not exist in this entry.");
            }
        } else {
